...
class HotelAddOnController extends Controller
{
    
    public function index($parent_id,Request $request)
    {
        if ($request->ajax()) {
            $data= Addon::query()->where('parent_id',Crypt::decrypt($parent_id))->where('type',1);
            ...
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('action', 'application.hotels.addons.action')
                ->make(true);
        }
        return view('application.hotels.addons.index',['hotel_id'=>$parent_id]);
    }

    public function create($parent_id)
    {
        return view('application.hotels.addons.create',['hotel_id'=>$parent_id]);
    }
    ...
    public function edit($parent_id,$id)
    {
        ...
        return view('application.hotels.addons.edit',['hotel_id'=>$parent_id,'data'=>$addOn]);
    }
